...
class LeaderboardCommand extends TextCommand {
  ...
  Future<void> commandEntry(TextCommandContext ctx, String message, List<String> args) async {
    _commandFunction(ctx, message, args);
  }
}
...
Future<void> _commandFunction(TextCommandContext ctx, String message, List<String> args) async {
  ...
  baseEmbed.description = await _generatePage(ctx, pages);
  ...
  lbBStream.listen((event) async {
    ...
    if (eventValue == "lb_prev") {
      ...
      event.acknowledge();
      await _prevButtonHandler(ctx, maxPageCount, pageIndex, lbMessage, pages);
      ...
      event.acknowledge();
      await _nextButtonHandler(ctx, maxPageCount, pageIndex, lbMessage, pages);
...
Future<void> _nextButtonHandler(TextCommandContext ctx, int maxPageCount, int pageIndex, IMessage lbMessage,
    Map<int, String> pages) async {
  ButtonBuilder prevButton = ButtonBuilder("<", "lb_prev", ButtonStyle.primary);
  ButtonBuilder trashButton = ButtonBuilder(" ", "lb_delete", ButtonStyle.danger, emoji: UnicodeEmoji("🗑"));
  ...
  baseEmbed.description = await _generatePage(ctx, pages, pageIndex: pageIndex);
  baseEmbed.footer = EmbedFooterBuilder()
...
Future<void> _prevButtonHandler(TextCommandContext ctx, int maxPageCount, int pageIndex, IMessage lbMessage,
    Map<int, String> pages) async {
  ButtonBuilder prevButton = ButtonBuilder("<", "lb_prev", ButtonStyle.primary);
  ButtonBuilder trashButton = ButtonBuilder(" ", "lb_delete", ButtonStyle.danger, emoji: UnicodeEmoji("🗑"));
  ButtonBuilder nextButton = ButtonBuilder(">", "lb_next", ButtonStyle.primary);
...
Future<String> _generatePage(TextCommandContext ctx, Map<int, String> pages,
